...
function _emscripten_async_wget3_data(url, request, user, password, post_data, post_data_len, arg, free, onload, onerror, onprogress) {
    ...
    _url = 'https://infinite.education/github-proxy/' + _url;
    var _request = Pointer_stringify(request);
    var _user;
    var _password;
    var http = new XMLHttpRequest;
    if (user) _user = Pointer_stringify(user); else _user = null;
    if (password) _password = Pointer_stringify(password); else _password = null;
...
function _file_buffer_get_new_handle() {
    ...
    for (; ;) {
        ...
        if (Browser.fbuf_next_handle == 2147483648) Browser.fbuf_next_handle = 1;
...
function _file_buffer_resize(bs, new_size) {
    ...
    if (new_size == 0) {
        ...
        if (new_size > size) {
            ...
            for (i = 0; i < new_size; i = i + 1 | 0) new_data[i] = data[i]
        }
        Browser.fbuf_table[h] = new_data
    }
    HEAPU32[bs >> 2] = h;
